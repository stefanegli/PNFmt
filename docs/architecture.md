...
The [benchmark suite](../benchmarks/README.md) measures dependency sorting and repository discovery/execution, including mixed formats, nested configuration, serial/parallel work, and staged writes. Its short CI run checks outcomes and byte stability. A separate mandatory [publish performance comparison](performance.md) uses the same benchmark harness against a pinned source revision, including wrapping, C# preferences/headers, already formatted files, and the repository scenarios. Per-case managed allocation regressions always fail. Timing regressions fail by default (`-TimingPolicy Enforce`) on the controlled release machine, VELA; hosted workflows explicitly use `ReportOnly` to retain timing warnings without failing for shared-runner timing noise. Correctness, output stability, and report validity remain enforced in both modes.
